...
// DiffuseMap
sampler2D Map__4Sampler = sampler_state
{ 
   Texture = <Map__4>; 
   MinFilter = Linear; 
   MagFilter = Linear; 
...
// SpecularMap
sampler2D Map__7Sampler = sampler_state
{ 
   Texture = <Map__7>; 
   MinFilter = Linear; 
   MagFilter = Linear; 
...
// NormalMap
sampler2D Map__5Sampler = sampler_state
{ 
   Texture = <Map__5>; 
   MinFilter = Linear; 
   MagFilter = Linear; 
...
float4  PS(VS_OUTPUT vout) : COLOR 
{
    ...
    float4 albedo = tex2D(Map__4Sampler, vout.TexCoord);
    ...
    if (diffuse.x > 0)
    {
        ...
        float4 specularIntensity = tex2D(Map__7Sampler, vout.TexCoord);
