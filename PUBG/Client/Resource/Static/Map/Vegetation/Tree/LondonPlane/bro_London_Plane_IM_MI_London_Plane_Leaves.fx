...
/*** dependency block ***/
texture MI_London_Plane_Leaves; 
texture Map__5; 
texture Map__4; 

sampler2D MI_London_Plane_LeavesSampler = sampler_state  // TexCoord0 
{ 
   Texture = <MI_London_Plane_Leaves>; 
   MinFilter = Linear; 
   MagFilter = Linear; 
...
float4  PS(VS_OUTPUT vout) : COLOR
{
    ...
    float4 albedo = tex2D(MI_London_Plane_LeavesSampler, vout.TexCoord);
    ...
    if (diffuse.x > 0)
    {
        ...
        float4 specularIntensity = tex2D(Map__5Sampler, vout.TexCoord);
