...
/*** dependency block ***/
texture MI_American_Elm_Leaves; 
texture Map__3; 
texture Map__5; 

sampler2D MI_American_Elm_LeavesSampler = sampler_state  // TexCoord0 
{ 
   Texture = <MI_American_Elm_Leaves>; 
   MinFilter = Linear; 
   MagFilter = Linear; 
...
sampler2D Map__5Sampler = sampler_state  // TexCoord0 
{ 
   Texture = <Map__5>; 
   MinFilter = Linear; 
   MagFilter = Linear; 
...
{
    float3 tangentNormal = tex2D(Map__5Sampler, vout.TexCoord).xyz;
    ...
    float4 albedo = tex2D(MI_American_Elm_LeavesSampler, vout.TexCoord);
