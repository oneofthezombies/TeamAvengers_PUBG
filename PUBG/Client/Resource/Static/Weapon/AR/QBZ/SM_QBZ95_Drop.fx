...
sampler2D Map__1Sampler = sampler_state  // DiffuseMap
{ 
   Texture = <Map__1>; 
   MinFilter = Linear; 
   MagFilter = Linear; 
...
sampler2D Map__3Sampler = sampler_state  // NormalMap
{ 
   Texture = <Map__3>; 
   MinFilter = Linear; 
   MagFilter = Linear; 
...
{
    float3 tangentNormal = tex2D(Map__3Sampler, vout.TexCoord).xyz;
    ...
    float4 albedo = tex2D(Map__1Sampler, vout.TexCoord);
